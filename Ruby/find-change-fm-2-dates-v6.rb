...
CSV.open(output_file, "wb") do |csv_out|
  ...
  CSV.foreach(input_file, headers: true) do |row|
    ...
    csv_row = [row['period'], row['name'], row['shares'], date1, date2, sprintf("%.2f", price1), sprintf("%.2f", price2), sprintf("%.2f", change), sprintf("%.2f", pct_change), number_with_delimiter(sprintf("%.2f", amount))]
    csv_out << csv_row

...
SQL
df1 = db_stock.fetch(sql, yesterday).all
...
# Display the results
puts "prd\tname\tshares\tu_cost\tprice\tdiv\tcost-%\tmrkt-%\teps   \t     dpr-%"
puts "123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890"
# df_merge.each do |row|
formatted_rows.each do |row|  
  puts "#{row[:prd]}\t#{row[:name]}\t#{row[:shares].to_s.reverse.gsub(/(\d{3})(?=\d)/, '\\1,').reverse}\t#{row[:u_cost]}\t#{row[:mkt_price]}\t#{row[:dividend]}\t#{row[:cst_percent]}\t#{row[:mkt_percent]}\t#{row[:eps]} \t#{row[:dpr_percent]}"
end

...
def summarize_prices(input_file, output_file)
    ...
    CSV.open(output_file, 'w') do |output|
        output << %w[name from_date to_date days from_price to_price spd ttl_qty max_price min_price trend nbr]
        header = %w[name fm_date to_date days fm_price to_price spd ttl_qty max_price min_price trend nbr].join('|')
        puts header
        ...
        CSV.foreach(input_file) do |row|
            ...
            if !previous_row.nil? # process for 2nd row onward
                if row[0] != previous_row[0]    # group break
                    ...
                    output << [previous_row[0], start_date, previous_row[1], days, start_price, previous_row[2], spd, ttl_qty, max_price, min_price, previous_trend, nbr]
                    output_string = [previous_row[0], start_date, previous_row[1], days, start_price, previous_row[2], spd, commas(ttl_qty), max_price, min_price, previous_trend, nbr].join(',')
                          puts output_string                    
                    start_date = row[1]
                    start_price = row[2]
                    max_price = row[4]
